...
def main():
    treePath = os.path.join(cwd, 'data/Twitter/Twitter_data_all.txt')
    print("reading tree")
    treeDic = {}
    ...
    labelPath = os.path.join(cwd, "data/Twitter/Twitter_label_all.txt")
    ...
    def loadEid(event, id, y):
        ...
        if len(event) < 2:
            return None
        if len(event) > 1:
            if os.path.exists(os.path.join(cwd, 'data/Twittergraph/' + id + '.npz')):
                pass
            else:
                # print('the id is ', id)
                x_features, x_senlen, tree_, root_feat, root_index = constructMat(event)
                rootfeat, tree, x_x, rootindex, y, senlen = root_feat.detach().numpy(), np.array(tree_), np.array(
                    x_features), np.array(
                    root_index), np.array(y), np.array(x_senlen)
                del x_features, x_senlen, tree_, root_feat, root_index
                gc.collect()
                np.savez(os.path.join(cwd, 'data/Twittergraph/' + id + '.npz'), x=x_x, root=rootfeat, edgeindex=tree,
                         rootindex=rootindex, y=y, seqlen=senlen)
    ...
    # Limit to first 100 events for faster processing
    max_samples = 100
    Parallel(n_jobs=3, backend='threading')(
        delayed(loadEid)(treeDic[eid] if eid in treeDic else None, eid, labelDic[eid]) for eid in tqdm(event[:max_samples]))
    return
